...
function classifyResultError(result) {
  ...
  if (!result.dryRun && result.runtimeObservedProvider && result.runtimeObservedModel && !result.modelAttested) {
...
server.tool(
  ...
  async ({ cwd }) => {
    ...
    const [openCodeVersion, gitVersion, agentDiscovery, safeOrchestratorDebug, contractorOrchestratorMetadata, contractorNestedAttestation, sanitizedReaderMetadata, managedSkillEvidence, providerCapacity] = await Promise.all([
      ...
      deep ? safeOpenCodeCommand(["debug", "agent", MCP_ORCHESTRATOR_AGENT], cwd, 1000 * 30) : Promise.resolve(null),
    ...
    const sanitizedReaderPolicyError = deep ? sanitizedAgentMetadataError(sanitizedReaderMetadata, path.resolve(cwd || process.cwd())) : null;
      ...
      content: [
        {
          ...
          text: [
            ...
            `MCP orchestrator edit permission denied: ${deep ? (safeOrchestratorPolicy?.tools?.apply_patch === false ? "yes" : "no") : "not checked in quick mode"}`,
            `MCP orchestrator nested task permission denied: ${deep ? (safeOrchestratorPolicy?.tools?.task === false ? "yes" : "no") : "not checked in quick mode"}`,
...
server.tool(
  ...
  async ({ pipelineId, cwd = "", confirmation, reason = "Operator abandoned an obsolete pipeline." }) => {
    ...
    const record = memoryRecord && recordMatchesProject(memoryRecord, projectRoot)
      ? memoryRecord
      : await readPersistedPipelineRecord(pipelineId, projectRoot);
    if (!record) {
      return { content: [{ type: "text", text: `Multi-agent pipeline not found: ${pipelineId}` }] };
    }
    ...
    const queueJobIds = Array.isArray(record.queueJobIds) ? record.queueJobIds : [];
    const childSnapshots = await Promise.all(
      queueJobIds.map((jobId) => readPersistedQueueRecord(jobId, projectRoot))
    );
    const children = {
      ok: queueJobIds.length === 0 ? record.status === "planned" : childSnapshots.every(Boolean),
      snapshots: childSnapshots.filter(Boolean),
    };
    if (!children.ok) {
      return { content: [{ type: "text", text: formatRejectedExecution({
        headline: "Multi-agent pipeline abandonment rejected.",
...
function pipelineRecordSnapshot(record) {
  ...
  return sanitizePersistedValue({
    ...
    queueMode: record.queueMode || effectiveQueueMode(),
...
async function runSelfTests() {
  ...
  let nonTransientGitReadAttempts = 0;
  const nonTransientGitRead = await runGitReadOnlyCommand(["status"], process.cwd(), 1000, async () => {
    nonTransientGitReadAttempts += 1;
    return { exitCode: 128, stdout: "", stderr: "fatal: not a git repository" };
  });
  assert.equal(nonTransientGitRead.exitCode, 128);
  assert.equal(nonTransientGitReadAttempts, 1);
