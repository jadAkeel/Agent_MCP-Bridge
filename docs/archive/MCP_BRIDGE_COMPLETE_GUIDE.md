...
For normal MCP calls, a requested OpenCode `orchestrator` is routed to the dedicated `opencode-orchestrator-mcp-planner`. It is read-only and has the OpenCode `task` permission denied, which technically blocks hidden nested subagents. Codex plans, coordinates, and invokes direct workers by default.

Three managed orchestrator profiles ship in `opencode/agents/`: `opencode-orchestrator-mcp-planner.md` (MCP planning target), `opencode-orchestrator-mcp-contractor.md` (explicit contractor target), and `opencode-orchestrator-standalone.md` (backup/standalone profile for direct OpenCode sessions). OpenCode names each agent after its file. The requested names `orchestrator`, `principal-engineer-orchestrator`, and `opencode-orchestrator-standalone` are bridge aliases that always resolve to the MCP planner or, in explicit contractor mode, to the contractor; the standalone profile is required in the managed tree but is never spawned by the bridge. `CODEX_OPENCODE_MCP_ORCHESTRATOR_AGENT` and `CODEX_OPENCODE_MCP_CONTRACTOR_ORCHESTRATOR_AGENT` override the two MCP target names for releases that intentionally ship different file names.
...
Per-job model selection: when the operator lists a model in `CODEX_OPENCODE_MODEL_ALLOWLIST`, a job whose `scopeContract.modelRequirement` names that model runs with an explicit `--model`/`--variant` pin instead of the profile pin, reported as `Model selection: operator_allowlist_override`. Requirements outside the allowlist are rejected before execution. Source-checkout dirt is governed by `CODEX_OPENCODE_SOURCE_DIRT_POLICY` (`strict` or `unrelated_ok`); see the README for both settings.
