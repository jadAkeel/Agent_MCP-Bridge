...
async function main() {
  ...
  try {
    ...
    assert.match(isolatedBuilderSource, /^model: openai\/gpt-5\.6-terra$/m, "Contractor E2E requires the production OpenAI Builder model.");
    assert.match(isolatedBuilderSource, /built-in Codex OAuth transport/, "Contractor E2E requires the production built-in OAuth policy.");
    assert.doesNotMatch(isolatedBuilderSource, /google\/antigravity|opencode\/big-pickle/, "Contractor E2E rejects a stale Builder plugin or fallback model.");
